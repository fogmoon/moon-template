	...
	cbpTooltipMenu.prototype = {
			...
			Array.prototype.slice.call( this.menuItems ).forEach( function( el, i ) {
				...
				if( self.touch ) {
					...
					trigger.addEventListener( 'click', function( ev ) {
						if( this.parentNode.querySelector( ' li > ul' ) ) {
							ev.preventDefault();
						}
					} );
		...
		_openMenu : function( el ) {
			...
			this.omtimeout = setTimeout( function() {
				var submenu = el.querySelector( 'li > ul' );
		...
		_handleClick : function( el, ev ) {
			var item = el.parentNode,
				...
				submenu = item.querySelector( 'li > ul' )

			// first close any opened one..
			if( typeof this.current !== 'undefined' &&  items.indexOf( item ) !== this.current ) {
				this._closeMenu( this.el.children[ this.current ] );
				this.el.children[ this.current ].querySelector( 'li > ul' ).setAttribute( 'data-open', 'false' );
			}
